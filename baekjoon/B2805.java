...
public class B2805 {
	...
	static int findSum (int[] treesHeightArray, int center) {
		int sum = 0;
		int [] array = treesHeightArray;
		for (int i: array) {
			i -= center;
			if(i > 0) {
				sum += i;
	...
	public static void main(String[] args) {
		...
		int necessaryHeight = scan.nextInt();
		int [] treesHeightArray = new int [theNumberOfTrees];
		for (int i = 0; i < treesHeightArray.length; i++) {
			treesHeightArray[i] = scan.nextInt();
		}
		
		// 내림차순
		Arrays.sort(treesHeightArray);
		
		// 높이 이진 탐색
		// 첫 범위 설정
		int min = 0;
		int max = treesHeightArray[treesHeightArray.length - 1];
		int result = 0;
		
		while(min < max) {
			int center = (min + max) / 2;
			
			int sum = findSum(treesHeightArray, center);
			int compareSum = findSum(treesHeightArray, (center + 1));

			// 선형 검색의 결과가 필요한 값과 같거나, 결과가 필요한 값보다 크지만 그 이상 늘릴 수 없는 경우 찾기
			if (sum == necessaryHeight || (sum > necessaryHeight && compareSum < necessaryHeight)) {
				result = center;
				break;
			} else if (sum > necessaryHeight) {
				min = center;
			} else {
				max = center;
			}
